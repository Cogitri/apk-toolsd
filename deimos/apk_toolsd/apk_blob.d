...
int apk_blob_for_each_segment(apk_blob_t blob, const(char)* split, apk_blob_cb cb, void* ctx);

void apk_blob_checksum(apk_blob_t b, const(EVP_MD)* md, apk_checksum* csum);
char* apk_blob_chr(apk_blob_t b, ubyte ch);

int apk_checksum_compare(const(apk_checksum)* a, const(apk_checksum)* b);
