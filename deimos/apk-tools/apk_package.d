...
struct apk_installed_package
{
    ...
    mixin(bitfields!(
        uint, "repository_tag", 6,
        uint, "run_all_triggers", 1,
        uint, "broken_files", 1,
        uint, "broken_script", 1,
        uint, "broken_xattr", 1,
        uint, "", 6));
}
...
struct apk_package
{
    ...
    mixin(bitfields!(
        uint, "repos", 32,
        uint, "marked", 1,
        uint, "uninstallable", 1,
        uint, "cached_non_repository", 1,
        uint, "", 29));
...
alias PKG_FILE_PRINTF = PKG_VER_PRINTF;

extern __gshared const(char)*[] apk_script_types;

void apk_sign_ctx_init (
    apk_sign_ctx* ctx,
    int action,
    apk_checksum* identity,
    int keys_fd);
void apk_sign_ctx_free (apk_sign_ctx* ctx);
int apk_sign_ctx_process_file (
    apk_sign_ctx* ctx,
    const(apk_file_info)* fi,
    apk_istream* is_);
...
int apk_deps_write (
    apk_database* db,
    apk_dependency_array* deps,
    apk_ostream* os,
    apk_blob_t separator);
